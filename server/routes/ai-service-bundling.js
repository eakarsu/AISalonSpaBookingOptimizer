// AI Smart service bundling
// AI recommends service combos that maximize upsell and satisfaction
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISalonSpaBookingOptimizer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISalonSpaBookingOptimizer. Focus area: Smart service bundling. ${`AI recommends service combos that maximize upsell and satisfaction`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Smart service bundling.\n${`AI recommends service combos that maximize upsell and satisfaction`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'service-bundling', context, parsed);
    res.json({ feature: 'service-bundling', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[service-bundling]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['service-bundling']);
